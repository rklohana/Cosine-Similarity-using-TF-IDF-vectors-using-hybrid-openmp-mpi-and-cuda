...
__global__ void cosineSimilarityKernel(double *dot_products, int a_size, int b_size, double *a_norms, double *b_norms, double *results) {
    ...
    for(int a_index = a_start; a_index < a_end; a_index++) {
        for(int b_index = b_start; b_index < b_end; b_index++) {
            results[a_index*b_size + b_index] = dot_products[a_index*b_size + b_index] / (a_norms[a_index] * b_norms[b_index]);
		//printf("\n\t\t\t\t\t\tgpu check: %f\n",results[a_index*b_size + b_index]);
        }
    }
}
...
double *cudaCosine(std::vector<std::vector<double> > v1, std::vector<std::vector<double> > v2) {
    ...
    std::cout<<std::endl<<"V1 vopied"<<std::endl;
    ...
    std::cout<<std::endl<<"V2 vopied"<<std::endl;
    ...
    cudaError_t cuder=  cudaDeviceSynchronize();
	if(cuder!=cudaSuccess){
	std::cout<<"kernel failed "<<cudaGetErrorString(cuder)<<"\n";
}
    ...
    cudaMemcpy(results, cu_dot_products, results_size, cudaMemcpyDeviceToHost);
	//std::cout<<results[0]<<"\n";	
    cudaFree(cu_v1_ptr);
    cudaFree(cu_v2_ptr);
